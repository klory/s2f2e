...
def read_and_decode(filename_queue):
    ...
    features={
        ...
        'sketch_raw': tf.FixedLenFeature([], tf.string),
        'neutral_raw': tf.FixedLenFeature([], tf.string)
        }
    )
    ...
    # [mnist.IMAGE_PIXELS].
    image = tf.decode_raw(features['sketch_raw'], tf.uint8)
    annotation = tf.decode_raw(features['neutral_raw'], tf.uint8)
